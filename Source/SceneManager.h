	...
	struct ComputerPart
    	{
    		...
    		ComputerPart(const glm::vec3 s, const glm::vec3 p, float y, const glm::vec4 c) 
    		: scaleXYZ(s), positionXYZ(p), rotationY(y), color(c) {}
    	};
...
public:
	...
	// Monitor
	void RenderMonitor();
	void RenderMonitorStand();
	void RenderMonitorFrame();
	
	// Keyboard
	void RenderKeyboardSimple();
	
	// Speakers
	void RenderSpeakers();
	void RenderSpeakerGrill();
};
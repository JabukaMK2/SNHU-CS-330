...
void ViewManager::PrepareSceneView()
{
	...
	// --- ALWAYS compute aspect from real framebuffer size (prevents stretching) ---
	int fbW = 0, fbH = 0;
	glfwGetFramebufferSize(m_pWindow, &fbW, &fbH);
	if (fbH == 0) fbH = 1;
	float aspect = (float)fbW / (float)fbH;

	// keep viewport synced with framebuffer
	glViewport(0, 0, fbW, fbH);

	// REDONE with if else statement for ortho switch
	...
	// else run as ortho
		...
		float s = m_orthoSize;
		projection = glm::ortho(-s * aspect, s* aspect, -s, s, 0.1f, 100.0f);
	}
	...
	if (NULL != m_pShaderManager)
	{
		...
		// COMPLETED: New if statement for if the camera is set ortho or not
		if (m_userPerspective) {

...
// Returns the surface color used for lighting when textures are enabled:
// (texture color) * (object tint)
vec3 GetTexturedTintedColor()
{
    return vec3(texture(objectTexture, fragmentTextureCoordinate * UVscale)) * objectColor.rgb;
}
...
{    
    if(bUseLighting == true)
    {
        ...
        if(bUseTexture == true)
        {
            // Match the same UVscale sampling used everywhere else
            float outA = texture(objectTexture, fragmentTextureCoordinate * UVscale).a * objectColor.a;
            fragmentColor = vec4(phongResult, outA);
        }
        else
        {
    ...
    else
    {
        if(bUseTexture == true)
        {
            fragmentColor = texture(objectTexture, fragmentTextureCoordinate * UVscale) * objectColor;
        }
        else
        {
...
vec3 CalcDirectionalLight(DirectionalLight light, vec3 normal, vec3 viewDir)
{
    ...
    if(bUseTexture == true)
    {
	vec3 texTint = GetTexturedTintedColor();
   	ambient  = light.ambient  * texTint;
    	diffuse  = light.diffuse  * diff * material.diffuseColor * texTint;

    	// Keep specular NOT tinted by the texture (looks better)
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    if(bUseTexture == true)
    {
    	...
    	// Keep specular not texture-tinted (recommended)
    	specular = light.specular * spec * material.specularColor;
    }
    else
    {
